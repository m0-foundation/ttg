...
contract SPOG_InitialState is SPOG_Base {
    ...
    function test_Revert_WhenSettingIncorrectInitialValues() public {
        ...
        new SPOG(initSPOGData, voteVault, valueVault, time, voteQuorum, valueQuorum, valueFixedInflationAmount, SPOGGovernorAbstract(payable(address(voteGovernor))), SPOGGovernorAbstract(payable(address(valueGovernor))));
        ...
        new SPOG(initSPOGData, voteVault, valueVault, time, voteQuorum, valueQuorum, valueFixedInflationAmount, SPOGGovernorAbstract(payable(address(0))), SPOGGovernorAbstract(payable(address(valueGovernor))));

        // revert valueGovernor is zero address
        vm.expectRevert("SPOGStorage: zero address");
        new SPOG(initSPOGData, voteVault, valueVault, time, voteQuorum, valueQuorum, valueFixedInflationAmount, SPOGGovernorAbstract(payable(address(voteGovernor))), SPOGGovernorAbstract(payable(address(0))));

        // revert voteGoverner and valueGovernor are the same
        vm.expectRevert("SPOGStorage: vote and value governor cannot be the same");
        new SPOG(initSPOGData, voteVault, valueVault, time, voteQuorum, valueQuorum, valueFixedInflationAmount, SPOGGovernorAbstract(payable(address(voteGovernor))), SPOGGovernorAbstract(payable(address(voteGovernor))));
    }
    ...
    function test_Revert_WhenSettingIncorrectVaultsInitValues() public {
        ...
        new SPOG(initSPOGData, IVoteVault(invalidVoteVault), valueVault, time, voteQuorum, valueQuorum, valueFixedInflationAmount, SPOGGovernorAbstract(payable(address(_voteGovernor))), SPOGGovernorAbstract(payable(address(_valueGovernor))));

        // revert vault value token is zero address
        address invalidValueVault = address(0);
        vm.expectRevert("SPOG: Vault address cannot be 0");
        new SPOG(initSPOGData, voteVault, IValueVault(invalidValueVault), time, voteQuorum, valueQuorum, valueFixedInflationAmount, SPOGGovernorAbstract(payable(address(_voteGovernor))), SPOGGovernorAbstract(payable(address(_valueGovernor))));
    }
