...
contract SPOG_InitialState is SPOG_Base {
    ...
    function test_Revert_WhenSettingIncorrectInitialValues() public {
        ...
        address voteVault = makeAddr("VoteVault");
        address valueVault = makeAddr("ValueVault");
        ...
        new SPOG(initSPOGData, voteVault, valueVault, time, voteQuorum, valueQuorum, valueFixedInflationAmount, ISPOGGovernor(voteGovernor), ISPOGGovernor(valueGovernor));
        ...
        new SPOG(initSPOGData, voteVault, valueVault, time, voteQuorum, valueQuorum, valueFixedInflationAmount, ISPOGGovernor(address(0)), ISPOGGovernor(valueGovernor));

        // revert valueGovernor is zero address
        vm.expectRevert("SPOGStorage: zero address");
        new SPOG(initSPOGData, voteVault, valueVault, time, voteQuorum, valueQuorum, valueFixedInflationAmount, ISPOGGovernor(voteGovernor), ISPOGGovernor(address(0)));

        // revert voteGoverner and valueGovernor are the same
        vm.expectRevert("SPOGStorage: vote and value governor cannot be the same");
        new SPOG(initSPOGData, voteVault, valueVault, time, voteQuorum, valueQuorum, valueFixedInflationAmount, ISPOGGovernor(voteGovernor), ISPOGGovernor(voteGovernor));
