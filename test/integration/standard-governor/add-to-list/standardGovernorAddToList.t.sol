...
contract StandardGovernorAddToList_IntegrationTest is IntegrationBaseSetup {
    function test_standardGovernorAddToList() external {
        ...
        callDatas_[0] = abi.encodeWithSelector(_standardGovernor.addToList.selector, bytes32("MintersList"), _dave);

        string memory description_ = "Add account to MintersList";
        ...
        vm.expectEmit();
        emit IRegistrar.AddressAddedToList("MintersList", _dave);
        ...
        assertTrue(_registrar.listContains("MintersList", _dave));
    }
}
