...
contract EpochBasedVoteToken is IEpochBasedVoteToken, ERC5805, ERC20Permit {
    ...
    function getVotes(address account_) external view virtual returns (uint256 votingPower_) {
        votingPower_ = _getLatestValue(_votingPowers[account_]);
    }

    function getPastVotes(address account_, uint256 epoch_) external view virtual returns (uint256 votingPower_) {
    ...
    function _delegate(address delegator_, address newDelegatee_) internal virtual override {
        ...
        _updateVotingPower(oldDelegatee_, _sub, votingPower_);
        _updateVotingPower(newDelegatee_, _add, votingPower_);
    }

    function _mint(address recipient_, uint256 amount_) internal virtual {
        emit Transfer(address(0), recipient_, amount_);

        _updateBalance(recipient_, _add, amount_);
        _update(_totalSupplies, _add, amount_);
        _updateVotingPower(_getDelegatee(recipient_), _add, amount_);
    }
    ...
    function _transfer(address sender_, address recipient_, uint256 amount_) internal virtual override {
        ...
        _updateBalance(sender_, _sub, amount_);
        _updateVotingPower(_getDelegatee(sender_), _sub, amount_);
