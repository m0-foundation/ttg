...
interface ISPOGVault {
    ...
    event EpochRewardsClaimed(uint256 indexed epoch, address indexed account, address indexed token, uint256 amount);
    ...
    function hasClaimedRewards(uint256 epoch, address token, address account) external view returns (bool);
}
