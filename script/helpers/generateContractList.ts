...
const getEphemeralContractName = (index: number) => {
  switch (index) {
    case 0:
      return "EmergencyGovernor";
    case 1:
...
export const generateContractList = (
  ...
  transactions.forEach(
    ({
      ...
      if (additionalContracts.length > 0 && contractName === "ZeroGovernor") {
        additionalContracts.forEach((additionalContract, index) => {
          contractList.contracts.push(
            formatContract(
              chainId,
              getEphemeralContractName(index),
              additionalContract.address,
            ),
          );
        });
      }
